...
public class MyOrder extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		order_title.setText("我的订单");

		/* initialize bmob java bean */
		Bmob.initialize(this, Constants.BMOB_API_ID);

		UserMessageInfo bmobUser = BmobUser.getCurrentUser(MyOrder.this,
	...
	private class OrderMessAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			LoadImage.LoadImageFile(MyOrder.this, Constants.LOADING_SAMLL_IMAGE,
			...
			holder.my_order_comment.setText("确认交易");
	...
	private void accomplishOrder(final int pos) {
		...
		builder.setNeutralButton("确定", new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				yesAccomplishpOrder(order.get(pos));
			}
		});
		builder.setNegativeButton("取消", null);
		builder.show();
	...
	protected void yesAccomplishpOrder(final UserOrder userOrder) {
		...
		updateOrder.update(this, userOrder.getObjectId(), new UpdateListener() {
		    ...
		    @Override
		    public void onFailure(int code, String msg) {
		    	Toast.makeText(getApplicationContext(), "操作失败！请稍后重试！", 0).show();
		    }
		});
	}
}
